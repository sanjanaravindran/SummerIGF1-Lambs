...
generated quantities {
  ...
  for (i in 1:N)
    Survival_rep[i] = bernoulli_logit_rng(alpha  + beta_IGF * IGF_true[i] + beta_Weight * Weight_sc[i] + beta_SexF * SexF[i] +
                                           // + beta_Twin * Twin[i] + beta_PopSize * PopSize_sc[i] + 
                                            sigma_t*t[BirthYear[i]] + sigma_u*u[MumID[i]]);
  vector[N] log_lik;
  for (i in 1:N) {
    log_lik[i] = bernoulli_logit_lpmf(Survival[i] | alpha  + beta_IGF * IGF_true[i] + beta_Weight * Weight_sc[i] +beta_SexF * SexF[i] +
                                           // + beta_Twin * Twin[i] + beta_PopSize * PopSize_sc[i] + 
                                            sigma_t*t[BirthYear[i]] + sigma_u*u[MumID[i]]);
  }
