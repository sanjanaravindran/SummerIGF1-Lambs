data {
  ...
  vector[N] Weight; //bodyweight response
  vector[N] BirthWt_sc; //bodyweight response
  vector[N] DaysSinceBirth_sc; //bodyweight response
...
parameters {
  ...
  real beta_BirthWt;
  real beta_DSB;
  real beta_MumAge1; // slope for mum age in weight~IGF + ... model
  real beta_MumAge2; // slope for mum age in weight~IGF + ... model
...
model {
 ...
 // likelihood
  Weight ~ normal(alpha + beta_IGF * IGF_true + beta_SexF * SexF + 
                    beta_Twin * Twin + beta_PopSize * PopSize_sc +
                    beta_BirthWt * BirthWt_sc + beta_DSB * DaysSinceBirth_sc + 
...
generated quantities {
  vector[N] Weight_rep;
  for (i in 1:N)
    Weight_rep[i] = normal_rng(alpha  + beta_IGF * IGF_true[i] + beta_SexF * SexF[i] + 
                                 beta_Twin * Twin[i] + beta_PopSize * PopSize_sc[i] + 
                                 beta_BirthWt * BirthWt_sc[i] + beta_DSB * DaysSinceBirth_sc[i] +
