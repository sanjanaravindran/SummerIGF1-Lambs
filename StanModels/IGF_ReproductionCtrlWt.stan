...
model {
  ...
  // likelihood
  BredAsAYearling ~ bernoulli_logit(alpha + beta_IGF * IGF_true + beta_Weight * Weight_sc + beta_SexF * SexF +
...
generated quantities {
  ...
  for (i in 1:N)
    BredAsAYearling_rep[i] = bernoulli_logit_rng(alpha  + beta_IGF * IGF_true[i] + beta_Weight * Weight_sc[i] + beta_SexF * SexF[i] +
