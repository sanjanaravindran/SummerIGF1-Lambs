...
model {
  ...
  BredAsAYearling ~ bernoulli_logit(alpha + beta_IGF * IGF_true + beta_SexF * SexF + 
                                      beta_Twin * Twin + 
                                      beta_PopSize * PopSize_sc + beta_Weight * Weight_sc +
...
generated quantities {
  ...
  for (i in 1:N)
    BredAsAYearling_rep[i] = bernoulli_logit_rng(alpha  + beta_IGF * IGF_true[i] + beta_SexF * SexF[i] + 
                                                   beta_Twin * Twin[i] + 
                                                   beta_PopSize * PopSize_sc[i] + beta_Weight * Weight_sc[i] +
                                                    beta_MumAge1 * MumAge_sc[i] + beta_MumAge2 * square(MumAge_sc[i]) +
                                                   sigma_t*t[BirthYear[i]] + sigma_u*u[MumID[i]]);
  vector[N] log_lik;
  for (i in 1:N) {
    log_lik[i] = bernoulli_logit_lpmf(BredAsAYearling[i] | alpha  + beta_IGF * IGF_true[i] + beta_SexF * SexF[i] + 
                                                   beta_Twin * Twin[i] + 
                                                   beta_PopSize * PopSize_sc[i] + beta_Weight * Weight_sc[i] +
                                                    beta_MumAge1 * MumAge_sc[i] + beta_MumAge2 * square(MumAge_sc[i]) +
                                                   sigma_t*t[BirthYear[i]] + sigma_u*u[MumID[i]]);
