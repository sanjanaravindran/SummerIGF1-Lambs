data {
  ...
  vector[N] ForeLeg; //bodyForeleg response
...
parameters {
  ...
  real alpha; //intercept for Foreleg~IGF + ... model
  real beta_IGF; //slope for IGF in Foreleg~IGF + ... model
  real beta_SexF; // slope for sex in Foreleg~IGF + ... model
  real beta_Twin; // slope for twin in Foreleg~IGF + ... model
  real beta_PopSize; // slope for pop size in Foreleg~IGF + ... model
  real beta_Weight;
  //real beta_AgeInDays;
  real beta_MumAge1; // slope for mum age in Foreleg~IGF + ... model
  real beta_MumAge2; // slope for mum age in Foreleg~IGF + ... model
...
model {
  ...
  // likelihood
  ForeLeg ~ normal(alpha + beta_IGF * IGF_true + beta_SexF * SexF + 
                    beta_Twin * Twin + beta_PopSize * PopSize_sc +
                    beta_Weight * Weight_sc +
...
generated quantities {
  vector[N] ForeLeg_rep;
  for (i in 1:N)
    ForeLeg_rep[i] = normal_rng(alpha  + beta_IGF * IGF_true[i] + beta_SexF * SexF[i] + 
                                 beta_Twin * Twin[i] + beta_PopSize * PopSize_sc[i] + 
                                 beta_Weight * Weight_sc[i] + 
